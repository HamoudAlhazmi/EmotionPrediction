...
with open(r"C:\Users\Hamou\OneDrive\المستندات\Term 2 2019 uc\Engineering Project A - 10004\sentiment 140\training.1600000.processed.noemoticon.csv") as f: #r means the string will be treated as raw string
   ...
   print (df.text[279]) #looks like HTML coding has not been converted to text. so we will decode HTML to general text and BeautifulSoup will be used for this.
   ...
   print(re.sub(r'@[A-Za-z0-9]+','',df.text[343]))# A-Za-z0-9 means it can be among the all the Uppercase and lowercase letters and the number betwween 0 and 9, and the letter
   ...
   pat1 = r'@[A-Za-z0-9_]+'#first define data cleaning function and then apply them to the whole dataset
   pat2 = r'https?://[^ ]+'
   ...
   def tweet_cleaner(text):
      ...
      try:
         bom_removed = souped.decode("utf-8-sig").replace(u"ï¿½", "?")
      except:
         bom_removed = souped
   ...
   # tok = WordPunctTokenizer()#first define data cleaning function and then apply them to the whole dataset
   # pat1 = r'@[A-Za-z0-9]+'
   ...
   #    # During the letters_only process two lines above, it has created unnecessay white spaces,
   #    # I will tokenize and join together to remove unneccessary white spaces
   #    words = tok.tokenize(lower_case)#Tokenizer converts Python text strings to streams of token objects
   #    return (" ".join(words)).strip()
